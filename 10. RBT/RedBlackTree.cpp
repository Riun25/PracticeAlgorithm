...
bool RedBlackTree::Insert(int data)
{
	...
	// 트리에 새 노드 추가.
	Insert(CreateNode(data, Color::Black));
	return true;
}
...
void RedBlackTree::RestructureAfterRemove(Node* node)
{
	...
	while (node->Parent() != nullptr && node->GetColor() == Color::Black)
	{
		...
		if (node == node->Parent()->Left())
		{
			...
			// Case3: 형제 노드 검정. 형제 노드의 왼쪽 자손이 Red
			// 해결 : 형제의 왼쪽 자식을 Black으로 변경, 형제의 Red로 변경 후 형제 기준 우회전
			...
			// Case4: 형제 노드는 Black, 형제의 오른쪽 자손이 Red
		...
		if (node == node->Parent()->Right())
		{
			...
			// Case3: 형제 노드 검정. 형제 노드의 오른쪽 자손이 Red
			// 해결 : 형제의 오른쪽 자식을 Black으로 변경, 형제의 Red로 변경 후 형제 기준 우회전
			if (sibling->Right()->GetColor() == Color::Red)
			{
				sibling->Right()->SetColor(Color::Black);
				sibling->SetColor(Color::Red);
			...
			// Case4: 형제 노드는 Black, 형제의 왼쪽 자손이 Red
			// 해결 : 형제를 부모와 같은 색으로 설정, 부모를 Black으로 변경, 형제의 왼쪽 자식을 Black으로 변경 후 부모 기준 좌회전
			if (sibling->Left()->GetColor() == Color::Red)
			{
				sibling->SetColor(sibling->Parent()->GetColor());
				sibling->Parent()->SetColor(Color::Black);
...
void RedBlackTree::RotateToRight(Node* node)
{
	...
	// 오른쪽 자식 노드의 오른쪽 자식 노드를 부모의 왼쪽 자식으로 등록
...
void RedBlackTree::RemoveImpl(Node* node)
{
	...
	{
		// 대체할 노드 검색 (왼쪽 하위 트리에서 가장 큰 값을 대체 노드로 설정)
...
void RedBlackTree::PrintRecursive(Node* node, int depth, int blackCount)
{
	...
	// 현재 노드 값 출력
	std::cout << node->Data() << " " << node->ColorString() << " [" << position <<
		", " << parentData << "] " << blackCountString << "\n";
